  ...
  return (
    <div className="mx-auto p-8 w-full max-w-screen-lg">
      <h1 className="text-2xl font-bold mb-4 flex items-center gap-2">
        Wishlist <FaHeart className="text-pink-400" />
      </h1>
      <div>
        {wishListItem.length ? (
          wishListItem.map((item) => {
            return (
              <div
                key={item.id}
                className="rounded-xl shadow-xl mb-4 flex flex-col sm:flex-row"
              >
                ...
                <div className="flex flex-col sm:flex-row justify-between w-full p-4">
                  <div className="mt-6">
                    <Link
                      ...
                      className="hover:text-gray-800 tracking-tighter transition duration-100 font-semibold md:text-lg lg:text-xl"
                    >
                      {item.name} - {item.category} - {item.shade} -{" "}
                      {item.material} - {item.brands}
                    </Link>
                    <div className="flex items-center gap-4 mt-2">
                      <p className="text-sm sm:text-base lg:text-lg">
                        {item.rating} /{" "}
                        <span className="text-[#00000077]">5</span>
                      </p>
